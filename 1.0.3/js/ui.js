class UIComponent {
    ...
    updateComponent() {
        ...
        let originX = (this.parent == this.canvasUI ? 0 : -this.width / 2);
        let originY = (this.parent == this.canvasUI ? 0 : -this.height / 2);

        switch (xComponent.type) {
            case 'REL':
                this.position.x = (this.parent.width * xComponent.value) + this.parent.position.x + originX;
                break;
            case 'LEFT':
                this.position.x = this.parent.position.x + xComponent.value + originX;
                break;
            case 'RITE':
                this.position.x = (this.parent.position.x + this.parent.width) - xComponent.value + originX;
                break;
        }

        switch (yComponent.type) {
            case 'REL':
                this.position.y = (this.parent.height * yComponent.value) + this.parent.position.y + originY;
                break;
            case 'TOP':
                this.position.y = this.parent.position.y + yComponent.value + originY;
                break;
            case 'BOT':
                this.position.y = (this.parent.position.y + this.parent.height) - yComponent.value + originY;
                break;
        }
    }
